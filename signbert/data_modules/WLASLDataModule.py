...
class WLASLDataModule(pl.LightningDataModule):
    DPATH = '/home/user/kimlab/Nakanishi/SignBERT/SLR/WLASL'
    SKELETON_DPATH = '/mnt/d-drive/user/kimlab/Nakanishi/npy/wlasl'
    ...
    def prepare_data(self):
        ...
        if not os.path.exists(WLASLDataModule.MEANS_FPATH) or \
            ...
            skeleton_fpaths = glob.glob(
                os.path.join(WLASLDataModule.TRAIN_SKELETON_DPATH, '*.npy')
            )
            train = [
                np.load(f)
        ...
        if not os.path.exists(WLASLDataModule.TRAIN_FPATH) or \
            ...
            not os.path.exists(WLASLDataModule.VAL_IDXS_FPATH):
            # Grab skeleton file paths
            ...
            val_skeleton_fpaths = glob.glob(
                os.path.join(WLASLDataModule.VAL_SKELETON_DPATH, '*.npy')
            )
            ...
            self._generate_preprocess_npy_arrays(
                ...
                WLASLDataModule.TRAIN_MAPPING_IDXS_FPATH
            )
            self._generate_preprocess_npy_arrays(
                val_idxs, 
                val_skeleton_fpaths, 
                ...
                WLASLDataModule.VAL_MAPPING_IDXS_FPATH
            )
